<template>
	<view class="product-con">
					...
					<view class='wrapper'>
						<view class='share acea-row row-between row-bottom'>
							<view class='money font-color'>
								...
								<image v-if="storeInfo.vip_price && storeInfo.vip_price > 0" :src="isrc+'vip.png'"></image>
							</view>
				...
				<view class='userEvaluation' id="past1">
					<view class='title acea-row row-between-wrapper'>
						...
						<navigator class='praise' hover-class='none' :url='"/pages/users/goods_comment_list/index?product_id="+id'>
							<text class='font-color-violet'>{{replyChance}}%</text>好评率
		...
		<view class='footer acea-row row-between-wrapper'>
			...
			<navigator class="animated item" :class="animated==true?'bounceIn':''" url='/pages/order_addcart/order_addcart'
			 hover-class="none">
		...
		<view class='poster-pop' v-if="posterImageStatus">
			<image :src='isrc+"poster-close.png"' class='close' @click="posterImageClose"></image>
		...
		<view class="share-box" v-if="H5ShareBox">
			<image :src="isrc+'share-info.png'" @click="H5ShareBox = false"></image>
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			...
			return {
				...
				posterImage: '', //海报路径
				posterbackgd: 'http://www.runcode.pro/goods/images/posterbackgd.png',
				sharePacket: {
					isState: true, //默认不显示
				}, //分销商详细
				uid: 0, //用户uid
...
<style scoped lang="scss">
	...
	.pictrue_log_class {
		...
		background: linear-gradient(90deg, #5F51B2 0%, #4B56A9 100%);
		opacity: 1;
		position: absolute;
		top: 0;
		left: 0;
