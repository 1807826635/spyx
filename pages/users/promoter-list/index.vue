			...
			<view class='list'>
				...
				<block v-for="(item,index) in recordList" :key="index">
					<view class='item acea-row row-between-wrapper'>
						...
						<view class="right">
							<view><text class='num font-color-violet'>{{item.childCount ? item.childCount : 0}}</text>人</view>
		...
		methods: {
			...
			userSpreadNewList: function() {
				...
				spreadPeople({
					...
					that.teamCount = res.data.count || 0;
