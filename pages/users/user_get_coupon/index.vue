	...
	<view>
		<view class='coupon-list' v-if="couponsList.length">
			<view class='item acea-row row-center-wrapper' v-for="(item,index) in couponsList" :key="index">
				...
				<view class='text'>
					<view class='condition acea-row row-middle'>
					    <view class='line-title' :class='(item.is_use==true || item.is_use==2)?"gray":""' v-if='item.type===0'>通用劵</view>
					    <view class='line-title' :class='(item.is_use==true || item.is_use==2)?"gray":""' v-else-if='item.type===1'>品类券</view>
					    <view class='line-title' :class='(item.is_use==true || item.is_use==2)?"gray":""' v-else>商品券</view>
		...
		<view class='noCommodity' v-else-if="!couponsList.length && loading==true">
			<view class='pictrue'>
				<image src='http://www.runcode.pro/goods/images/noCoupon.png'></image>
			</view>
		</view>
		<!-- #ifdef MP -->
