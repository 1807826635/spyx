		...
		<view class='order-submission'>
			<view class="allAddress" :style="store_self_mention ? '':'padding-top:10rpx'">
				<view class="nav acea-row">
					<view class="item font-color-violet" :class="shippingType == 0 ? 'on' : 'on2'" @tap="addressType(0)" v-if='store_self_mention'></view>
					<view class="item font-color-violet" :class="shippingType == 1 ? 'on' : 'on2'" @tap="addressType(1)" v-if='store_self_mention'></view>
				</view>
				<view class='address acea-row row-between-wrapper' @tap='onAddress' v-if='shippingType == 0'>
					<view class='addressCon' v-if="addressInfo.real_name">
						...
						<view class="line1">
							<text class='default font-color-violet' v-if="addressInfo.is_default">[默认]</text>{{addressInfo.province}}{{addressInfo.city}}{{addressInfo.district}}{{addressInfo.detail}}</view>
...
<style lang="scss" scoped>
	...
	.order-submission .allAddress .nav .item.on2::before {
		...
		border-style: none solid solid;
		border-color: transparent transparent #c5c9e3;
