						...
						<div class="item acea-row row-top" v-if="item.uid !== uid" :key="item.id">
			                ...
			                path: '/pages/admin/orderDetail/index?id=' + item.orderInfo.order_id
									...
									<div class="conter acea-row row-middle" v-if="item.msn_type === 4">
										<img :src="isrc+'signal2.gif'" class="signal" style="margin-right: 0.27rem;" />12’’
									</div>
		...
		<div class="footerCon" :class="active === true ? 'on' : ''" :style="'transform: translate3d(0,' + percent + '%,0);'" ref="footerCon">
			<form>
				<div class="footer acea-row row-between row-bottom" ref="footer">
					...
					<img @click="uploadImg" :src="isrc+'plus.png'" />
					<img :src="
              active === true
                ? 'http://www.runcode.pro/goods/images/keyboard.png'
                : 'http://www.runcode.pro/goods/images/face.png'
            "
					 @click="emoticon" />
				 ...
				 v-if="emojiGroup.length > 0">
					<block v-for="(emojiList, index) in emojiGroup" :key="index">
						<swiper-item>
							...
							<img :src="isrc+'del.png'" class="emoji-outer" />
						</swiper-item>
					</block>
