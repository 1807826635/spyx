<template>
	<view>
		<view class="address-window" :class="address.address==true?'on':''">
			...
			<view class='list'>
				<view class='item acea-row row-between-wrapper' :class='active==index?"font-color-violet":""' v-for="(item,index) in addressList"
				 @tap='tapAddress(index,item.id)' :key='index'>
					<text class='iconfont icon-ditu' :class='active==index?"font-color-violet":""'></text>
					<view class='address'>
						<view class='name' :class='active==index?"font-color-violet":""'>{{item.real_name}}<text class='phone'>{{item.phone}}</text></view>
						<view class='line1'>{{item.province}}{{item.city}}{{item.district}}{{item.detail}}</view>
					</view>
					<text class='iconfont icon-complete' :class='active==index?"font-color-violet":""'></text>
				</view>
			</view>
			<!-- 无地址 -->
			<view class='pictrue' v-if="!is_loading && !addressList.length">
				<image src='http://www.runcode.pro/goods/images/noAddress.png'></image>
			</view>
